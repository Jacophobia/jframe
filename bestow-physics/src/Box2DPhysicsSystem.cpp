// bestow-physics/src/Box2DPhysicsSystem.cpp
...
// Convert bestow BodyType to Box2D body type
...
// Convert Box2D body type to bestow BodyType
...
bool Box2DPhysicsSystem::initialize() {
    ...
    // Bestow uses Y-down screen coordinates (positive Y = down on screen)
    // Box2D uses Y-up coordinates (negative Y = down in physics)
    // So we pass gravity directly - Bestow's positive Y gravity becomes Box2D's positive Y
