...
bool Game::initialize(bestow::core::Engine& engine) {
    bestow::core::logInfo("Initializing GAS Demo Game");
    ...
    levelAssetHandle_ = sys.assets->registerAsset(
        bestow::AssetType::Level, "data/levels/level1.lua");
...
void Game::updateFixed(bestow::DeltaTime dt) {
    ...
    if (sys.physics->hasBody(player_)) {
        ...
        for (bestow::Entity zone : sys.entities->collect<JumpZoneTag>()) {
            if (sys.physics->hasBody(zone)) {
                bestow::Vec2 zonePos = sys.physics->getPosition(zone);
        ...
        if (inJumpZone_ && !wasInZone && jumpZoneTag) {
            ...
            bestow::core::logInfo("Entered jump zone - you can jump now!");
        } else if (!inJumpZone_ && wasInZone && jumpZoneTag) {
            gas_->removeTag(player_, *jumpZoneTag);
            bestow::core::logInfo("Left jump zone - jumping disabled");
        }
    }
}
...
void Game::render(float alpha) {
    ...
    if (debugFrames < 5) {
        if (debugFrames == 0) {
            ...
            size_t withBoth = 0;
            for (bestow::Entity e : sys.entities->view<bestow::DebugRect>()) {
                withDebugRect++;
                if (sys.entities->tryGet<bestow::Transform2D>(e)) {
                    withBoth++;
                }
            }
            bestow::core::logInfo("DEBUG: Entities with DebugRect: " + std::to_string(withDebugRect));
            bestow::core::logInfo("DEBUG: Entities with Transform2D + DebugRect: " + std::to_string(withBoth));
        }

        // Print camera info each frame
        if (camera_) {
            auto cam = camera_->getCamera();
            bestow::core::logInfo("DEBUG frame " + std::to_string(debugFrames) + ": Camera pos: (" +
...
void Game::shutdown() {
    bestow::core::logInfo("Shutting down GAS Demo");
...
void Game::setupAudio() {
    ...
    bestow::core::logInfo("Audio system initialized with " + std::to_string(sounds.size()) + " sounds");
}
...
void Game::playSound(const std::string& soundName) {
    ...
    bestow::Channel channel = SFX_CHANNEL;
...
void Game::setupGAS() {
    ...
    bestow::core::logInfo("Loading GAS definitions from Lua...");
    std::ifstream file("data/config/abilities.lua");
    if (!file) {
        bestow::core::logError("Could not open data/config/abilities.lua");
        return;
    }
    ...
    if (!gas_->loadDefinitionsFromLua(buffer.str())) {
        bestow::core::logError("Failed to load GAS definitions");
        return;
    }
    ...
    if (healthDef && staminaDef && speedDef) {
        ...
        bestow::core::logInfo("GAS attributes loaded");
    }
    ...
    if (dashDef && jumpDef) {
        ...
        bestow::core::logInfo("GAS abilities loaded");
    }
    ...
    if (regenDef && stunDef) {
        ...
        bestow::core::logInfo("GAS effects loaded");
    }
    ...
    gas_->setAbilityActivatedCallback([](const bestow::AbilityActivatedEvent& e) {
        bestow::core::logInfo("Ability activated!");
    });
}
...
void Game::setupBlueprints() {
    ...
    auto safeGetFloat = [](const bestow::PropertyMap& props, const std::string& key, float defaultVal) -> float {
        auto it = props.find(key);
        if (it == props.end()) return defaultVal;
        try {
    ...
    auto safeGetString = [](const bestow::PropertyMap& props, const std::string& key, const std::string& defaultVal) -> std::string {
        auto it = props.find(key);
        if (it == props.end()) return defaultVal;
        try {
    ...
    auto safeGetBool = [](const bestow::PropertyMap& props, const std::string& key, bool defaultVal) -> bool {
        auto it = props.find(key);
        if (it == props.end()) return defaultVal;
        try {
    ...
    blueprints_->registerComponent("EnemyTag",
        [safeGetString, safeGetInt](bestow::Entity e, bestow::IEntitySystem& sys, const bestow::PropertyMap& props) {
            EnemyTag tag;
    ...
    blueprints_->registerComponent("WalkerAI",
        [safeGetFloat](bestow::Entity e, bestow::IEntitySystem& sys, const bestow::PropertyMap& props) {
            WalkerAI ai;
    ...
    blueprints_->registerComponent("CollectableTag",
        [safeGetString](bestow::Entity e, bestow::IEntitySystem& sys, const bestow::PropertyMap& props) {
            CollectableTag tag;
    ...
    blueprints_->registerComponent("HealthPickup",
        [safeGetInt](bestow::Entity e, bestow::IEntitySystem& sys, const bestow::PropertyMap& props) {
            HealthPickup hp;
    ...
    blueprints_->registerComponent("Projectile",
        [safeGetInt, safeGetBool](bestow::Entity e, bestow::IEntitySystem& sys, const bestow::PropertyMap& props) {
            Projectile proj;
    ...
    if (blueprints_->loadBlueprints(luaSource)) {
        bestow::core::logInfo("Loaded " + std::to_string(blueprints_->getBlueprintNames().size()) + " blueprints");
    } else {
        bestow::core::logError("Failed to parse blueprints from entities.lua");
    }
}
...
void Game::createPlayer() {
    ...
    sys.entities->emplace<bestow::DebugRect>(player_, bestow::DebugRect{
        .size = {playerPhysicsWidth_, playerPhysicsHeight_},
        .fillColor = bestow::Color{50, 200, 50, 255},      // Green
        .outlineColor = bestow::Color{25, 100, 25, 255},   // Dark green outline
        .outlineWidth = 2.0f,
    ...
    bestow::core::logInfo("Player created at (" + std::to_string(spawnX) + ", " + std::to_string(spawnY) + ")");
}
...
void Game::loadLevel() {
    ...
    for (const auto& def : entityDefs) {
        ...
        bestow::Entity entity = blueprints_->create(blueprintName, def.transform.x, def.transform.y, width, height);

        if (!sys.entities->isValid(entity)) {
            bestow::core::logWarn("Failed to create entity from blueprint: " + blueprintName);
            continue;
        }
    ...
    bestow::core::logInfo("Level loaded with " + std::to_string(platformCount) +
...
void Game::checkCollectablePickups() {
    ...
    for (bestow::Entity collectable : collectables) {
        if (!sys.physics->hasBody(collectable)) {
            continue;
        }

        bestow::Vec2 collectPos = sys.physics->getPosition(collectable);
        ...
        if (distance < 40.0f) {
            ...
            if (collectableTag) {
                ...
                if (abilityDef) {
                    ...
                    bestow::core::logInfo("Collected " + collectableTag->abilityToGrant + " ability!");
...
void Game::handlePlayerInput(bestow::DeltaTime dt) {
    ...
    if (sys.input->wasActionJustPressed("dash") && !isDashing_) {
        if (gas_->canActivateAbility(player_, dashAbility_)) {
            ...
            if (cooldown > 0.0f) {
                bestow::core::logInfo("Dash on cooldown");
            } else if (!hasDashAbility_) {
                bestow::core::logInfo("Dash not unlocked - find the cyan collectable");
            } else {
                bestow::core::logInfo("Cannot dash (not enough stamina or stunned)");
            }
        }
    }
...
void Game::renderUI() {
    ...
    sys.graphics->drawRect({10, 35, 200, 20}, bestow::Color{50, 50, 50, 255}, true);
    sys.graphics->drawRect({10, 35, staminaBarWidth, 20}, bestow::Color{0, 255, 255, 255}, true);
    sys.graphics->drawRect({10, 35, 200, 20}, bestow::Color::white(), false);
    ...
    if (dashCooldown > 0.0f) {
        ...
        sys.graphics->drawRect({10, 60, 50, 50}, bestow::Color{100, 100, 100, 255}, true);
        sys.graphics->drawRect({10, 60, 50, cooldownHeight}, bestow::Color{255, 0, 0, 128}, true);
    } else {
        sys.graphics->drawRect({10, 60, 50, 50}, bestow::Color{0, 255, 0, 255}, true);
    }
    sys.graphics->drawRect({10, 60, 50, 50}, bestow::Color::white(), false);
    ...
    for (const auto& effect : activeEffects) {
        ...
        if (effectDef) {
            bestow::Color effectColor = bestow::Color{128, 128, 255, 255};
            if (effectDef->name == "DashSpeedBoost") {
                effectColor = bestow::Color{0, 255, 255, 255};
            } else if (effectDef->name == "HealthRegen") {
                effectColor = bestow::Color{0, 255, 0, 255};
            } else if (effectDef->name == "Stun") {
                effectColor = bestow::Color{128, 0, 128, 255};
            }

            sys.graphics->drawRect({10, effectY, 100, 20}, effectColor, true);
            sys.graphics->drawRect({10, effectY, 100, 20}, bestow::Color::white(), false);
...
void Game::updateSwitches() {
    ...
    for (bestow::Entity sw : sys.entities->view<SwitchTag>()) {
        ...
        bestow::Vec2 swPos = sys.physics->getPosition(sw);
        ...
        if (dist < 30.0f) {
            ...
            for (bestow::Entity door : sys.entities->view<DoorTag>()) {
                ...
                if (doorTag && doorTag->doorId == switchTag->targetDoorId) {
                    ...
                    playSound("doorOpen");
                    bestow::core::logInfo("Door " + std::to_string(doorTag->doorId) + " opened!");
                }
            }
            bestow::core::logInfo("Switch activated!");
        }
    }
}
...
void Game::updateMovingPlatforms(bestow::DeltaTime dt) {
    auto& sys = engine_->systems();

    // Use entity query to iterate moving platforms
    for (bestow::Entity plat : sys.entities->view<MovingPlatform>()) {
...
void Game::updateProjectiles(bestow::DeltaTime dt) {
    ...
    for (bestow::Entity proj : projectiles) {
        ...
        // Move projectile
        bestow::Vec2 pos = sys.physics->getPosition(proj);
        ...
        if (projData->isEnemyProjectile) {
            ...
            for (bestow::Entity enemy : sys.entities->view<EnemyTag>()) {
                if (!sys.physics->hasBody(enemy)) continue;
                bestow::Vec2 enemyPos = sys.physics->getPosition(enemy);
    ...
    for (bestow::Entity trigger : sys.entities->view<TriggerZone>()) {
        ...
        if (playerPos.x >= triggerPos.x - halfW && playerPos.x <= triggerPos.x + halfW &&
            ...
            if (zone->event == "boss_start") {
                ...
                bestow::core::logInfo("BOSS FIGHT STARTED!");
            } else if (zone->event == "victory") {
                bestow::core::logInfo("VICTORY! You beat the game!");
            }

            bestow::core::logInfo("Trigger: " + zone->event);
        }
    }
}
...
void Game::checkHealthPickups() {
    ...
    for (bestow::Entity pickup : healthPickups) {
        auto* hp = sys.entities->tryGet<HealthPickup>(pickup);
        if (!hp || !sys.physics->hasBody(pickup)) {
            continue;
        }

        bestow::Vec2 pickupPos = sys.physics->getPosition(pickup);
        ...
        if (std::sqrt(dx * dx + dy * dy) < 30.0f) {
            ...
            bestow::core::logInfo("Health restored! Health: " + std::to_string(playerHealth_));
        }
    }
}
...
void Game::checkCheckpoints() {
    ...
    for (bestow::Entity cp : sys.entities->view<Checkpoint>()) {
        ...
        bestow::Vec2 cpPos = sys.physics->getPosition(cp);
        ...
        if (std::sqrt(dx * dx + dy * dy) < 40.0f) {
            ...
            bestow::core::logInfo("Checkpoint activated at (" + std::to_string(cpPos.x) + ", " + std::to_string(cpPos.y) + ")");
        }
    }
}
...
void Game::handleDoubleJump() {
    ...
    if (sys.input->wasActionJustPressed("jump")) {
        if (gas_->canActivateAbility(player_, doubleJumpAbility_)) {
            ...
            bestow::core::logInfo("Double Jump!");
...
void Game::handleWallJump() {
    ...
    if (sys.input->wasActionJustPressed("jump")) {
        if (gas_->canActivateAbility(player_, wallJumpAbility_)) {
            ...
            playSound("wallJump");
            bestow::core::logInfo("Wall Jump!");
            gas_->endAbility(player_, wallJumpAbility_);
        }
    }
}

void Game::handleGroundPound(bestow::DeltaTime dt) {
    auto& sys = engine_->systems();
    ...
    if (sys.input->wasActionJustPressed("ground_pound") && isInAir_ && !isGroundPounding_) {
        if (gas_->canActivateAbility(player_, groundPoundAbility_)) {
            ...
            bestow::core::logInfo("Ground Pound!");
    ...
    if (isGroundPounding_ && isGrounded_) {
        ...
        for (bestow::Entity br : breakables) {
            if (!sys.physics->hasBody(br)) continue;
            bestow::Vec2 brPos = sys.physics->getPosition(br);
            ...
            if (std::sqrt(dx * dx + dy * dy) < 80.0f) {
                ...
                bestow::core::logInfo("Breakable destroyed!");
            }
        }

        bestow::core::logInfo("Ground Pound impact!");
    }
}
...
void Game::createProjectile(float x, float y, float dirX, float dirY, bool isEnemy) {
    ...
    bestow::Entity proj = blueprints_->create(blueprintName, x, y, 12.0f, 12.0f);

    if (!sys.entities->isValid(proj)) {
        bestow::core::logWarn("Failed to create projectile");
        return;
    }
...
void Game::handleCombat(bestow::DeltaTime dt) {
    auto& sys = engine_->systems();
    ...
    if (sys.input->wasActionJustPressed("attack") && !isAttacking_ && hasSwordAbility_) {
        ...
        bestow::AbilityId attackAbility = swordAttackAbility_;
        ...
        if (gas_->canActivateAbility(player_, attackAbility)) {
            ...
            bestow::core::logInfo("Sword attack! Combo: " + std::to_string(comboCount_));
    ...
    if (sys.input->wasActionJustPressed("shield") && hasShieldAbility_) {
        if (gas_->canActivateAbility(player_, shieldAbility_)) {
            ...
            bestow::core::logInfo("Shield activated!");
    ...
    if (sys.input->wasActionJustPressed("ranged") && hasRangedAbility_) {
        if (gas_->canActivateAbility(player_, rangedAbility_)) {
            ...
            bestow::core::logInfo("Ranged attack!");
...
void Game::checkSwordCollisions() {
    ...
    if (bossActive_) {
        ...
        if (bossOpt && sys.physics->hasBody(*bossOpt)) {
            bestow::Vec2 bossPos = sys.physics->getPosition(*bossOpt);
...
void Game::playerTakeDamage(int amount) {
    ...
    if (shieldTag && gas_->hasTag(player_, *shieldTag)) {
        ...
        bestow::core::logInfo("Shield blocked some damage!");
    }
    ...
    bestow::core::logInfo("Player took " + std::to_string(amount) + " damage! Health: " + std::to_string(playerHealth_));
    ...
    if (playerHealth_ <= 0) {
        ...
        bestow::core::logInfo("Player died! Respawning at checkpoint...");
    }
}

void Game::enemyTakeDamage(bestow::Entity enemy, int amount) {
    auto& sys = engine_->systems();
    ...
    playSound("enemyHit");
    bestow::core::logInfo("Enemy took " + std::to_string(amount) + " damage! Health: " + std::to_string(enemyTag->health));
    ...
    if (enemyTag->health <= 0) {
        ...
        enemiesDefeated_++;
        bestow::core::logInfo("Enemy defeated! Total: " + std::to_string(enemiesDefeated_));
    }
}
...
void Game::bossTakeDamage(int amount) {
    bossHealth_ -= amount;
    playSound("bossHurt");
    bestow::core::logInfo("Boss took " + std::to_string(amount) + " damage! Health: " + std::to_string(bossHealth_));

    // Phase transitions
    if (bossHealth_ <= 350 && bossPhase_ == 1) {
        bossPhase_ = 2;
        bestow::core::logInfo("Boss entering phase 2!");
    } else if (bossHealth_ <= 150 && bossPhase_ == 2) {
        bossPhase_ = 3;
        bestow::core::logInfo("Boss entering phase 3!");
    }
    ...
    if (bossHealth_ <= 0) {
        ...
        bestow::core::logInfo("BOSS DEFEATED! Victory!");
    }
}

void Game::updateEnemies(bestow::DeltaTime dt) {
    auto& sys = engine_->systems();
...
void Game::updateEnemy(bestow::Entity enemy, bestow::DeltaTime dt) {
    auto& sys = engine_->systems();
    ...
    if (enemyTag->type == "walker") {
        ...
        if (walker) {
            bestow::Vec2 vel = sys.physics->getVelocity(enemy);
...
void Game::updateBoss(bestow::DeltaTime dt) {
    ...
    bestow::Entity boss = *bossOpt;

    bossAttackTimer_ -= dt;
    bossPatternTimer_ -= dt;

    bestow::Vec2 bossPos = sys.physics->getPosition(boss);
    bestow::Vec2 playerPos = sys.physics->getPosition(player_);
    ...
    // Move toward player
    bestow::Vec2 vel = sys.physics->getVelocity(boss);
...
void Game::bossPatternAttack() {
    ...
    bestow::core::logInfo("Boss attack pattern!");
}
...
void Game::bossPatternJump() {
    ...
    bestow::Vec2 vel = sys.physics->getVelocity(*bossOpt);
    vel.y = -500.0f;
    sys.physics->setVelocity(*bossOpt, vel);
    bestow::core::logInfo("Boss jump!");
}
...
void Game::bossPatternSummon() {
    ...
    blueprints_->create("WalkerEnemy", bossPos.x + 100.0f, bossPos.y, 32.0f, 32.0f);
    bestow::core::logInfo("Boss summoned minions!");
}
