// jframe-dev/src/ComponentRegistry.cpp
...
struct ComponentInfo {
    std::string name;
    std::function<bool(const JFrameEngine&, Entity)> hasComponent;
    std::function<std::string(const JFrameEngine&, Entity)> serialize;
};
...
public:
    ...
    std::string serializeComponent(const JFrameEngine& engine, Entity entity,
...
private:
    ...
    void registerStandardComponents() {
        ...
        registerComponent(
            "Transform2D",
            [](const JFrameEngine& engine, Entity e) {
                return engine.entities && engine.entities->allOf<Transform2D>(e);
            },
            [](const JFrameEngine& engine, Entity e) -> std::string {
